...
the `ap:*` (Platform API) and `dp:*` (Developer Portal) namespaces.

No admin credentials are shipped — `APIP_CP_ADMIN_USERNAME` and `APIP_CP_ADMIN_PASSWORD_HASH` are
required and have no fallback, so startup fails until you set them (generate a hash with
`htpasswd -bnBC 12 "" <password> | tr -d ':\n'`). The quickstart `setup.sh` generates both into
`api-platform.env` for you. Alternatively, set `platform_api.auth.mode = "internal_token"`
